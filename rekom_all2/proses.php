...
if(isset($_POST['add'])){
	...
	$status= trim(mysqli_real_escape_string($con, $_POST['status']));
	mysqli_query($con, "UPDATE tb_rekom SET  id_user='$user', rekomendasi='$rekomendasi', deadline='$deadline', komentar_ki='$komentar_ki' , status='$status' WHERE id_rekom='$id'") or die (mysqli_error($con));
	echo "<script>window.location='data.php';</script>";
}
?>